 ...
 */
 ...
 import { Dataset as DatacookDataset, Dataset, Image } from '@pipcook/datacook';
 // @ts-ignore
 ...
 */
const imageClassDataCollect: DataSourceEntry<Dataset.Types.Sample, Dataset.Types.ImageDatasetMeta> = async (options: Record<string, any>, context: ScriptContext) => {
  const {
    url = ''
  } = options;
  ...
  const meta = {
    type: Dataset.Types.DatasetType.Image,
    size: {
      train: train.length,
      test: train.length
    },
